...
if( input() == '0' ):
    ...
    IV, cipher = modeCTR( IV, key )
    cipherText = encryptMessage( cipher, message.encode() ) 
    saveEncrypt( IV, key, cipherText, "CTR" )

else:
    KeyandIV, cipherText = obtainContent( "CTR" )
    ...
    file = open( location + "MessageDecryptCTR.txt", "w" )
